...
If the signer rejected the request, it MUST send `RESULT_OK` with a `rejected` field set to `true` instead of using `RESULT_CANCELED`, so the client can distinguish between a user rejection and a system cancellation (e.g. the user pressing the back button):
